...
export function registerCatalogueTools(server: McpServer, http: HttpClient): void {
  server.registerTool(
    "list_catalogues",
    {
      ...
      description:
        ...
        "- synchronize_allowed, stop_allowed, delete_allowed: AUTHORITATIVE permission flags for the next operation. Always trust these over reading the status yourself — they already encode the single-bottleneck rule (sync and search cannot run simultaneously across the whole system).\n" +
        ...
        "- busy → '*_failed' (crashed; retryable via synchronize_catalogue)\n" +
  ...
  server.registerTool(
    "add_catalogue",
    {
      ...
      description:
        "Registers a folder so the backend knows about it. This is pure bookkeeping — it does NOT start indexing. The returned catalogue has status=new. To actually index the images, call synchronize_catalogue afterwards.\n\n" +
