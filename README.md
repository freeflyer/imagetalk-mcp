...
Note that using this MCP **makes search partly non-local**: the image search itself still runs against your local backend, but the queries and match metadata pass through the remote LLM driving the chat, unlike the native ImageTalk frontend which stays fully on your machine.
...
- **Catalogue lifecycle**: `list_catalogues`, `get_catalogue`, `add_catalogue`, `synchronize_catalogue`, `stop_catalogue_sync`, `detach_catalogue`
